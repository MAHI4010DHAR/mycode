...
#------------------------------------------------------------------------------
# Class: VSSFxNLMS algorithm with initial coefficients determined by SFANC
#------------------------------------------------------------------------------
class VSSFxNLMS():
...
class SFANC_VSSFxNLMS():
    ...
    def noise_cancellation(self, Dis, Fx, filter_index, Stepsize):
        Error = []
        j = 0
        model = VSSFxNLMS(Len=1024, Ws=self.Current_Filter)
        ...
        for ii, dis in enumerate(Dis):
            ...
            if (ii + 1) % self.fs == 0:
                ...
                if self.Current_Filter[0].equal(self.Wc[filter_index[j]]) == False:
                    ...
                    print('change the initial weights of VSSFxNLMS')
